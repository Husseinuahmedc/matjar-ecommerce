...
export default async function SupportOrderDetailPage({ params }: PageProps) {
  ...
  const order = await getOrderById(id, "demo-user", "SUPPORT")
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <Link
              href={`/${locale}/support/orders`}
              className="text-muted-foreground hover:text-foreground transition-colors"
            >
              {isAr ? <ArrowRight className="h-5 w-5" /> : <ArrowLeft className="h-5 w-5" />}
            </Link>
            <h1 className="text-2xl font-bold">{isAr ? "تفاصيل الطلب" : "Order Details"}</h1>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {order.events && order.events.length > 0 && (
            <div className="rounded-xl border bg-card shadow-sm p-6">
              <h3 className="font-bold mb-4">{isAr ? "سجل الطلب" : "Order Timeline"}</h3>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border bg-card shadow-sm p-6 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">{isAr ? "تكاليف الشحن" : "Shipping"}</span>
            ...
            <div className="pt-2 border-t flex justify-between items-center">
              <span className="text-sm text-muted-foreground">{isAr ? "حالة الدفع" : "Payment Status"}</span>
